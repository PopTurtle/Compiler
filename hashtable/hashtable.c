...
void hashtable_foreach(const hashtable *ht, void (*callback)(const void *keyref, const void *valref)) {
  if (ht == NULL || callback == NULL) {
    return; // Si la table ou le callback est NULL, on ne fait rien
  }

  // Parcourir tous les compartiments du tableau de hachage
  size_t m = POW2(ht->lbnslots);  // Nombre de compartiments (tableau de hachage)
  for (size_t i = 0; i < m; ++i) {
    // Parcourir la liste chaînée associée à chaque compartiment
    cell *p = ht->hasharray[i];
    while (p != NULL) {
      // Appeler la fonction de rappel pour chaque élément
      callback(p->keyref, p->valref);
